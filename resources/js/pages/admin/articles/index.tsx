...
export default function AdminArticleIndex({ articles, filters, statuses, summary, authors, categories, tags }: Props) {
    ...
    const resetFilters = () => {
        router.get('/admin/articles', {}, { preserveState: true, preserveScroll: true, replace: true });
    };
    ...
    const publish = (id: number) => router.post(`/admin/articles/${id}/publish`);
    const approve = (id: number) => router.post(`/articles/${id}/approve`);
    const reject = (id: number) => router.post(`/articles/${id}/reject`);
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="grid gap-6 p-6">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">Manage All Articles</h1>
                        <p className="text-muted-foreground text-sm">Admin datatable with filters for all article records.</p>
                    </div>
                    <Button asChild>
                    ...
                    <CardContent className="grid gap-3 p-4 md:grid-cols-8">
                        ...
                        <div>
                            ...
                            <Input
                                ...
                                className="mt-1"
                                onChange={(e) => applyFilter({ from_date: e.target.value || undefined })}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-1 flex gap-2">
                                <Input
                                    ...
                                    onChange={(e) => applyFilter({ to_date: e.target.value || undefined })}
                                />
                                                    ...
                                                    <td className="py-3 pr-0 align-top">
                                                        <div className="flex justify-end gap-2">
                                                            ...
                                                            {article.permissions?.approve && article.status === 'pending' && (
                                                                <Button size="sm" variant="outline" onClick={() => approve(article.id)}>Approve</Button>
                                                            )}
                                                            {article.permissions?.reject && article.status === 'pending' && (
                                                                <Button size="sm" variant="outline" onClick={() => reject(article.id)}>Reject</Button>
                                                            )}
                                                            {article.permissions?.publish && article.status !== 'published' && (
                                                                <Button size="sm" variant="outline" onClick={() => publish(article.id)}>Publish</Button>
                                                            )}

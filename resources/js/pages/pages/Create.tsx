...
export default function CreatePage({ categories }: Props) {
    ...
    const submit = (e: React.FormEvent) => { e.preventDefault(); post('/pages'); };
                ...
                <div className="sticky top-0 z-20 flex items-center justify-between border-b bg-background/95 px-6 py-3 backdrop-blur">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-muted-foreground text-xs truncate max-w-48">
                            {data.title ? `"${data.title.slice(0, 45)}${data.title.length > 45 ? '…' : ''}"` : 'New page'}
                        </span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Button variant="outline" size="sm" type="button" asChild>
                            <Link href="/pages">Discard</Link>
                        </Button>
                        <Button
                            size="sm" type="button" variant="outline"
                            onClick={() => { setData('status', 'draft'); setTimeout(() => post('/pages'), 0); }}
                            disabled={processing}
                        >
                            Save Draft
                        </Button>
                        <Button
                            size="sm" type="button"
                            onClick={() => { setData('status', 'published'); setTimeout(() => post('/pages'), 0); }}
                            disabled={processing}
                            className="gap-1.5"
                        >
                            ...
                            <TabsContent value="seo" className="space-y-5 mt-0">
                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div className="space-y-1.5">
                                        ...
                                        />
                                        {errors.meta_title && <p className="text-destructive text-xs">{errors.meta_title}</p>}
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <Input
                                            ...
                                            placeholder="https://…"
                                        />
                                        {errors.canonical_url && <p className="text-destructive text-xs">{errors.canonical_url}</p>}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Input
                                        ...
                                        placeholder="https://… (used for social sharing previews)"
                                    />
                                </div>
                                ...
                                <div className="rounded-xl border bg-muted/20 p-4">
                                    ...
                                    <div className="space-y-0.5">
                                        <p className="text-xs text-green-700 dark:text-green-400">/page/{data.slug || 'page-slug'}</p>
                                        <p className="text-base font-medium text-blue-700 dark:text-blue-400 hover:underline cursor-default line-clamp-1">
                            ...
                            <TabsContent value="settings" className="space-y-5 mt-0">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Input
                                        ...
                                        className="w-28"
                                    />
                                    <p className="text-muted-foreground text-xs">Lower numbers appear first in menu listings.</p>
                                </div>
                            </TabsContent>
                        </Tabs>
                    </div>
                    ...
                    <div className="space-y-4 xl:sticky xl:top-[57px] xl:self-start">
                        ...
                        <Card className="border shadow-none">
                            ...
                            <CardContent className="space-y-3 px-4 pb-4">
                                ...
                                <div className="flex gap-2">
                                    <Button
                                        type="button" variant="outline" size="sm" className="flex-1 text-xs"
                                        onClick={() => { setData('status', 'draft'); setTimeout(() => post('/pages'), 0); }}
                                        disabled={processing}
                                    >
                                        Save Draft
                                    ...
                                    <Button
                                        type="button" size="sm" className="flex-1 text-xs gap-1"
                                        onClick={() => { setData('status', 'published'); setTimeout(() => post('/pages'), 0); }}
                                        disabled={processing}
                                    >
                                ...
                                {data.status === 'published' && (
                                    <a href={`/page/${data.slug}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-primary hover:underline">
                                        <Eye className="size-3" /> Preview live page
                                    </a>
                                )}
                            </CardContent>
                        </Card>
                        ...
                        <Card className="border shadow-none">
                            ...
                            <CardContent className="space-y-3 px-4 pb-4">
                                ...
                                {imgError && (
                                    <p className="text-muted-foreground text-xs">Could not load image preview.</p>
                                )}
                                {errors.featured_image && <p className="text-destructive text-xs">{errors.featured_image}</p>}
                            </CardContent>
                        </Card>

...
import type { Article, BreadcrumbItem, Category, Paginated, Tag } from '@/types';
...
export default function ArticleIndex({ articles, filters, statuses, categories, tags }: Props) {
    const stats = useMemo(() => {
        const counters = { total: articles.total, draft: 0, pending: 0, published: 0, rejected: 0 };
        articles.data.forEach((article) => {
            if (article.status in counters) {
                counters[article.status as keyof typeof counters] += 1;
            }
        });
        return counters;
    }, [articles]);
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="grid gap-6 p-6">
                ...
                <div className="grid gap-4 md:grid-cols-5">
                    ...
                    <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Draft</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{stats.draft}</div></CardContent></Card>
                    ...
                    <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Published</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{stats.published}</div></CardContent></Card>
                    <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Rejected</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{stats.rejected}</div></CardContent></Card>
                </div>
